...
class BuyerAuthController(private val buyerService: BuyerService) {
    ...
    fun register(@RequestBody body: RegisterDto): ResponseEntity<User> {
        ...
        return ResponseEntity.ok(buyerService.save(user))
    }

    @PostMapping("/login")
    fun login(@RequestBody body: LoginDto, response: HttpServletResponse): ResponseEntity<Any> {
        val user = this.buyerService.findByEmail(body.email)
    ...
    fun user(request: HttpServletRequest): ResponseEntity<Any> {
        ...
        try {
            ...
            return ResponseEntity.ok(this.buyerService.getById(body.issuer.toInt()))
